...
export default function ResultSection({ displayScore, getScoreColor, getScoreLabel }) {
  return (
    <div
      className="bg-linear-to-br from-white/20 via-white/60 to-white/20 rounded-3xl shadow-2xl px-6 sm:px-8 py-8 sm:py-12 border-2 border-white backdrop-blur-xl flex flex-col items-center hover:shadow-3xl transition-all duration-300"
    ...
    >
      ...
      <div className="flex items-end gap-2 sm:gap-3">
        <span
          ...
          style={{
            ...
            textShadow: `0 10px 30px ${getScoreColor(displayScore)}40, 0 0 60px ${getScoreColor(displayScore)}20`
          }}>
          {displayScore}
        </span>
      ...
      <div className="relative w-full h-4 sm:h-5 bg-linear-to-r from-slate-100 to-slate-200 rounded-full overflow-hidden border border-white/40">
        <div
          className="absolute h-full rounded-full transition-all duration-1000 ease-out shadow-lg"
          style={{
